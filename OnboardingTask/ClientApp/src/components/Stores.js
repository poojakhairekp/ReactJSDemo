...
export class StoreList extends Component {
    ...
    handleSave(event) {
        ...
        if (this.state.id != 0) {
            const response1 = fetch('api/Stores/' + this.state.id, options1)
        }
        else {
            const response2 = fetch('api/Stores', options2)
        }
    }
    ...
    handleDelete(id) {
        if (!window.confirm("Are you sure you want to delete this record with store number: " + id)) {
            return;
        }
        else {
